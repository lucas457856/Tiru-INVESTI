// Modal de confirmação para EXCLUSÃO DEFINITIVA de DONO.
//
// Espelha o padrão visual e o fluxo de `FuncionarioExcluirDefinitivoModal`,
// mas a operação é mais ampla: apaga o DONO do Firebase Authentication,
// remove o perfil, e TODOS os clientes, contratos, funcionários,
// sub-coleções, modelos, notificações e configurações vinculados a ele
// (excluindo o que cada funcionário criou também).
//
// Fluxo:
//   1. Usuário clica no botão "Excluir" (ícone de lixeira) na tabela
//      de donos do Painel Administrativo.
//   2. Modal abre com aviso destacado de ação permanente.
//   3. Usuário digita o NOME EXATO do dono para confirmar.
//   4. Clica em "Excluir definitivamente".
//   5. POST /api/admin/delete-owner com { donoUid }.
//   6. Sucesso → fecha modal e mostra toast verde de confirmação.
...
// no servidor. A sessão do ADMIN nunca é afetada.

import { useState } from "react";
import { AlertTriangle, LoaderCircle, Trash2, X } from "lucide-react";
import { excluirDono } from "../services/adminService";

export default function DonoExcluirDefinitivoModal({
  aberto,
  dono,
  aoFechar,
  aoSucesso,
  ...
  const nomeEsperado = (dono.nome || "").trim();
  const podeExcluir = confirmacao.trim() === nomeEsperado && nomeEsperado.length > 0;
  ...
  async function handleExcluir() {
    ...
    setErro("");
    setSalvando(true);
    const resp = await excluirDono(dono.uid);
    setSalvando(false);
    if (!resp || !resp.ok) {
      // Mostra a mensagem real do backend (sem try/catch que esconda).
      setErro(resp?.erro || "Não foi possível excluir o dono.");
      return;
    }
    aoSucesso?.(resp);
  }
      ...
      >
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                Você está prestes a excluir o dono{" "}
                <span className="font-bold text-slate-700 dark:text-slate-200">
                  {nomeEsperado || "(sem nome)"}
                </span>
                .
              </p>
            </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-red-700 dark:text-red-300 leading-relaxed">
              ...
              <p className="mt-1.5">
                ...
                relacionados a este usuário, incluindo:
              </p>
        ...
        <div className="mt-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-3 py-2.5 text-xs text-slate-600 dark:text-slate-300">
          ...
          <ul className="mt-1.5 space-y-1 list-disc list-inside">
            <li>Conta de autenticação do dono (Firebase Authentication)</li>
            <li>Perfil e cadastro do dono</li>
            <li>Todos os clientes (cadastrados pelo dono e por seus funcionários)</li>
            <li>Todos os contratos e seus históricos de pagamentos e juros</li>
            <li>Todos os funcionários, suas contas de autenticação e dados criados por eles</li>
            <li>Modelos de contrato e modelos de cobrança</li>
            <li>Notificações, dispositivos e demais configurações do dono</li>
          </ul>
        </div>
        ...
        <div className="mt-4">
          <label className="text-xs font-semibold text-slate-600 dark:text-slate-300">
            Tem certeza que deseja continuar? Para confirmar, digite{" "}
            <span className="font-mono text-slate-900 dark:text-white">
              {nomeEsperado || "(nome do dono)"}
            </span>
          </label>
          <input
            type="text"
            ...
            disabled={salvando}
            autoFocus
            placeholder={nomeEsperado || "Digite o nome do dono"}
